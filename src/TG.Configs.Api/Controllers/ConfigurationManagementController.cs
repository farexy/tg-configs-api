...
namespace TG.Configs.Api.Controllers
{
    ...
    public class ConfigurationManagementController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new CreateConfigCommand(
                request.Id, request.Content, Parse(request.Format), request.HasSecrets, User.GetEmail()));
            return result.ToActionResult()
                .Created();
        }
        ...
        public async Task<ActionResult<ConfigManagementResponse>> Update([FromRoute] string id, [FromBody] ConfigRequest request)
        {
            var result = await _mediator.Send(new UpdateConfigCommand(
                id, request.Content, Parse(request.Format), request.HasSecrets, User.GetEmail()));
            return result.ToActionResult()
                .NotFound(AppErrors.NotFound)
                .Ok();
        }
